...
var $ez=array('fontSize'=>24); // used for storing most of the page configuration parameters
...
function Cezpdf($paper='C5',$orientation='portrait'){
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array(
    'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>12,'titleFontSize'=>14
  ...
  $middle = ($this->ez['pageWidth']-$this->ez['rightMargin'])/2.0+($this->ez['leftMargin'])/2.0;
  ...
  foreach($data as $row){
    foreach($cols as $colName=>$colHeading){
      $w = $this->ezPrvtGetTextWidth($options['fontSize'],(string)$row[$colName])*0.20;
  ...
  foreach($cols as $colName=>$colTitle){
    $w = $this->ezPrvtGetTextWidth($options['fontSize'],(string)$colTitle)*0.20;
    if ($w > $maxWidth[$colName]){
      $maxWidth[$colName]=$w;
    }
  }
...
function ezImage($image,$pad = 0,$width =0,$resize = 'full',$just = 'left',$border = ''){
