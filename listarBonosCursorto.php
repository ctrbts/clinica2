...
if($_POST['fechaBono']==''){
			$url_relativa = "consultaCurso.php?valor=1";
		...
		$cadenaFecha.='-';	
		$cadenaFecha.="".$dia."";
		$consulta="select * from bonoCursorto where fecha = '$cadenaFecha'";
		...
		$pdf->ezText($data,12,array('justification'=>'center'));	
		 ...
		 while ($fila=mysql_fetch_assoc($sql)){
				 ++$valor;
				 $dni=$fila['dni'];
				 $consuper="select * from pacienteOrto where dni = $dni";
				 $sqlper=mysql_query($consuper);
				 $filaper=mysql_fetch_assoc($sqlper);
				 $nyap=$filaper['nyap'];
			//	 echo $nyap;
				 $idCurso=$fila['idCurso'];
				 $conscurso=" select * from cursorto where idCurso = $idCurso";
				 $sqlcurso=mysql_query($conscurso);
				 $filacurso=mysql_fetch_assoc($sqlcurso);
				 $curso=$filacurso['curso'];
				 $fechaBaseDeDatos=$fila['fecha'];
				 $bono=$fila["idBono"];
				 $data[] = array('Numero'=>$fila["idBono"],'Paciente'=>$filaper['nyap'], 'DNI'=>$fila["dni"], 'curso'=>$filacurso["curso"], 'Fecha'=>$fila["fecha"],'Hora'=>$fila["hora"]);

		}
		$titles = array('Numero'=>'<b>#BONO</b>','Paciente'=>'<b>PACIENTE</b>', 'DNI'=>'<b>DNI</b>','curso'=>'<b>CURSO</b>', 'Fecha'=>'<b>FECHA</b>','Hora'=>'<b>HORA</b>');
