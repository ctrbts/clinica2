...
<html>
	...
	<body>
	   ...
	   <?php if (!isset($_SESSION['pass'])){ 
				...
				<div > 
				<?php	
								$consudatos="select * from pacienteorto where dni =$dni";
							...
							<?php  //listar la curso y poner en que curso esta
												...
												$consulta="select * from bonocursorto where dni = $dni order by idBono Desc limit 1 ";
											...
											<input type="submit" value="GUARDAR" name="GUARDAR" >&nbsp;&nbsp;<input onClick="location.href = 'bonoPacienteCursorto.php?valor=0' " type="button" value="VOLVER atras" name="CANCELAR" ><br />
			  ...
			  /* if($valor == 1){ ?>
				   ...
				   if($valor == 3){    //aca iria la carga del bono y redireccionamiento al bono por primera ves
											...
											$consulta="select * from pacienteOrto where dni = '$dni'" ;
											$result=mysql_query($consulta);
											$fila=mysql_fetch_assoc($result);
										
											$consultacurso="select * from cursorto where idCurso='$curso'";
											...
											$consulBono="INSERT INTO bonocursorto (dni, idCurso, valor,fecha, hora, cancelado) values ('$dni','$curso', '$precio', '$fecha', '$hora', 0)";
											$resul=mysql_query($consulBono);			
											$url="bonoCursortoPdf.php?dni=$dni";
											redirigir($url);
						  }
		}	
	else {
