...
<?php if (!isset($_SESSION['pass'])){ 
			...
			//tengo que obtener el ultimo bono de esterilizacion que agregue para esa persona
			 $consultaBono="SELECT * FROM escaneodeboca where dni= '$dni' ORDER BY idBono DESC LIMIT 1";
			 $resuBono=mysql_query($consultaBono);
			 $filabonocurso=mysql_fetch_assoc($resuBono);
			 $numeroBono=$filabonocurso['idBono'];
			 $idServ=$filabonocurso['idServ'];
			 //la consulta para sacar el nombre del curso y el vamor
			 $consulserv="select * from servicio where idServ = $idServ";
			 $sqlserv=mysql_query($consulserv);
			 $filaserv=mysql_fetch_assoc($sqlserv);
			 
			 $nombreservicio=$filaserv['servicio'];
			 //echo $nombrecurso;
			 $valor=$filaserv['valor'];
			  
			 //echo $numeroBono;
			 //consulta para obtener con el dni el nombre y apellido del paciente
			 $consultaPaciente="select * from pacientescaneo where dni = '$dni'";
		...
		$pdf->ezText( "<b>            </b>" .$nombreservicio. "    ",18,array('justification'=>'left'));
