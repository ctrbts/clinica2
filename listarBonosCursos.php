...
if($_POST['fechaBono']==''){
		...
		$cadenaFecha.='-';	
		$cadenaFecha.="".$dia."";
		$consulta="select * from bonoCurso where fecha = '$cadenaFecha'";
		...
		$pdf->ezText($data,12,array('justification'=>'center'));	
		 ...
		 while ($fila=mysql_fetch_assoc($sql)){
				 ++$valor;
				 $dni=$fila['dni'];
				 $consuper="select * from pacienteCurso where dni = $dni";
				 $sqlper=mysql_query($consuper);
				 $filaper=mysql_fetch_assoc($sqlper);
				 $nyap=$filaper['nyap'];
			//	 echo $nyap;
				 $idCurso=$fila['idCurso'];
				 $conscurso=" select * from curso where idCurso = $idCurso";
