			 ...
			 <div> <img src="imagenes/banner.jpg" style="width:750px; height:100px; position:relative; left:-250px" /> </div> 								<spaN style="position: relative; right:-50px "><p style="color:#FF0000"><span><?php echo $_SESSION["usuario"] ?></span><div style="position:relative; top:-36px; right:-110px" >&nbsp;&nbsp; &nbsp;<a href="cerrarsesion.php"><p style="color:#0000CC">[ CERRAR SESION ]</p></span></p></a>  </span>	
					...
					if ( $valor == 0 ) { 
					 ?>
									...
									<form action="cargarPacienteCurso.php?codi=0" method="POST" enctype="multipart/form-data" id="carga" name="carga" >	
						...
						<?php
						       ...
						       if ($valor == 1){ //aca va si el persona es nuevo con codi igual a 1
													...
													$sqlcurso=mysql_query($consultacurso);
													 ...
													 $dni=$_GET['dni']; ?>
													<form action="cargarPacienteCurso.php?codi=2" method="POST" enctype="multipart/form-data" target="_blank">	
									   ...
									   <div style="margin-left:28%">	<input  type="submit" value="GUARDAR" name="GUARDAR" /> &nbsp;&nbsp;<input onClick="location.href = 'bonoCurso.php?valor=0' " type="button" value="CANCELAR" name="CANCELAR" /></div><br/>
																		<?php 
																		//desconectamos la BD
																		desconectar();	?>		
													</form>
							...
							<?php } 
										...
										if ($valor == 2){
													...
													$dni=$_GET['dni'];
													//echo $dni;
													$consuBonoUltimo="select * from bonoCurso where dni = $dni order by idBono desc limit 1";	
													$resulta=mysql_query($consuBonoUltimo);
													$filaUlti=mysql_fetch_assoc($resulta);
													$idCurso=$filaUlti['idCurso'];
													// Ejecutar Consulta
													$consultacurso="select * from curso where idCurso = $idCurso";
													$sqlcurso=mysql_query($consultacurso);
													$filaCurso=mysql_fetch_assoc($sqlcurso);
													$curso=$filaCurso['curso'];  //nombre del curso	
													$consulnyap="select * from pacienteCurso where dni = $dni";
													...
													<form action="verinfoPacienteCurso.php?valor=3" method="POST" enctype="multipart/form-data" target="_blank">	
													  ...
													  <?php     
														   ...
														   $cons="select * from bonocurso where dni = $dni order by idBono Desc limit 1 ";
														   $sqlcur=mysql_query($cons);
												           $fila3=mysql_fetch_assoc($sqlcur);
												           $cursoActual= $fila3['idCurso'];
														   
														   $consunomcurso="select * from curso where idCurso = $cursoActual";
									   ...
									   <div style="margin-left:28%">	<input  type="submit" value="GUARDAR" name="GUARDAR" /> &nbsp;&nbsp;<input onClick="location.href = 'bonoPacienteCurso.php?valor=0' " type="button" value="CANCELAR" name="CANCELAR" /></div><br/> 
																		<?php 
																		//desconectamos la BD
																		desconectar();	?>		
													</form>  
