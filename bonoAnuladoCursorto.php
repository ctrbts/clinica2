...
<html>
	...
	<head>
		...
		<title>Reimpresion de bonos del curso de ortodoncia Cancelados</title>
		<style type="text/css">
...
<?php
  ...
  if (!isset($_SESSION['pass'])){  
		 $bono=$_POST['bono'];
		 $db = conectar_admin();
				$consu="UPDATE `bonoCursorto` SET `cancelado`= 1 WHERE `idBono` = $bono";	
		  ...
		  if ($cancelado = 1){
				$dni=$fila['dni'];
		  		$consupersona="select * from pacienteOrto where dni = $dni";
		  ...
		  ?></p> <p style=" font-size:30px; color:#FF0000" > <?php   
				...
				//Aca tendria que poner la reimpresion
				 ?>		<div style="position: relative; left:12%"><p style="font-size:18px; color:#1383F3; position:relative; left:-100px;">Clickee en Reimprimir bono para poder imprimir el bono cancelado</p></div>									
						<div >
							<a href="reimprimirbonoCursortoAnuladoPdf.php?bono=<?php echo $bono ?>"><p style="font-size:25px; color:#003300">Reimprimir Bono</p></a>
						</div>
						<?php 
