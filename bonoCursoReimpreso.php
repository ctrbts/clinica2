...
<?php
  ...
  if (!isset($_SESSION['pass'])){  
		 $bono=$_POST['bono'];
		 $db = conectar_admin();
				/*$consu="Update bono Set cancelado=1 Where numeroBono = $bono";	
		  ...
		  <?php
			$consul="select * from bonocurso where idBono=$bono";
			$resultado=mysql_query($consul);
			$fila=mysql_fetch_assoc($resultado);
			$cancelado=$fila['cancelado'];
			$dni=$fila['dni'];
			$consultaPac="select * from pacienteCurso where dni = $dni";
			$sqlpac=mysql_query($consultaPac);
		  ...
		  ?> <p style=" font-size:30px; color:#FF0000" > <?php   
		  		echo 'Nombre y apellido: ';  echo   $filaPac['nyap']; ?>
				<br/>
				<?php
		...
		?> </fieldset >
			<div style="position:relative; left:-200px; top:10px"><a href="bonoCurso.php?valor=0">Volver </a>
		 ...
		 desconectar();
			...
			<div style="position:relative; left:1050px; top:-80px; width:100px; heigh:100px; right:100px"><img src="imagenes/UNLP_Logo.jpg" alt="logo unlp" /></div>
			 <div style="position:relative; left:30px; right:30px; top:-220px; " ><fieldset style=" width:950px; heigh:200px; background-color:#D2F525; border-top-width: 10px;  border-right-width: 1em;  border-bottom-width: thick;  border-left-width: thin; color:#00CC33;">
						 <legend align="center" style="font-size:40px; "><B style="left:100px; color:#ff8DF5 ">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;REIMPRIMIR BONO CURSO</B></legend>
		  ...
		  if ($cancelado = 1){  
		  		?><p style="font-size:18px; color:#000099"> <?php echo 'Nombre y apellido: ';  echo $filaPac['nyap']; ?>
				<br/>
		  ...
		  ?></p> <p style=" font-size:30px; color:#FF0000" >
						<div >
							<a href="bonoCursoReimpresoPdf.php?bono=<?php echo $bono ?>" target="_blank"><p style="font-size:25px; color:#003300">Reimprimir Bono </p></a>
						</div>
						<?php 
