...
var $ez=array('fontSize'=>14); // used for storing most of the page configuration parameters
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array(
    'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>10
  ...
  foreach($data as $row){
    foreach($cols as $colName=>$colHeading){
      $w = $this->ezPrvtGetTextWidth($options['fontSize'],(string)$row[$colName])*2.00;
  ...
  foreach($cols as $colName=>$colTitle){
    $w = $this->ezPrvtGetTextWidth($options['fontSize'],(string)$colTitle)*4.00;
    if ($w > $maxWidth[$colName]){
      $maxWidth[$colName]=$w;
    }
  }
